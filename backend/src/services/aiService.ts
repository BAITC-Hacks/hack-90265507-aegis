...
const fields=["brand","productType","poles","current","voltage","article"];
...
export async function interpretMessage(message:string,context:unknown):Promise<Plan>{
  ...
  for(const field of fields){const v=(p.requirements as unknown as Record<string,unknown>)[field];
    if(["poles","current","voltage"].includes(field)){if(typeof v==="number"&&Number.isFinite(v)&&v>0&&v<1e7)Object.assign(requirements,{[field]:v});}
