...
export function parseCatalogOptions(params:Record<string,unknown>):CatalogOptions {
  ...
  return {query:text("q")||"",category:text("category",100),brand:text("brand",80),productType:text("productType",100),city:text("city",80),
    page:number("page",1,1000000,true)||1,limit:number("limit",1,100,true)||24,scan:number("scan",1,600,true),
...
export async function verifyCandidates(ids:number[],loader=getProductById,onVerified?:(product:VerifiedProduct,index:number)=>void) {
  const result:(VerifiedProduct|undefined)[]=new Array(ids.length).fill(undefined);
  let next=0;
  await Promise.all(Array.from({length:Math.min(4,ids.length)},async()=>{
    for(;;){const index=next++;if(index>=ids.length)return;try{const product=await loader(ids[index]);onVerified?.(product,index);result[index]=product;}catch{ /* Expose failures through coverage, never fabricate fields. */ }}
  }));
  return result;
}
export async function searchVerifiedCatalog(options:CatalogOptions,loader=getProductById) {
  const {page=1,limit=24,sort="relevance",scan}=options;
  ...
  const selected=strict?(scan===undefined?candidates:candidates.slice(0,scan)):candidates.slice((page-1)*limit,page*limit);
  // Validate each response when received; a complete scan can exceed the cache lifetime.
  // Preserve verifiedAt so clients can display the actual observation time.
  const observations=new Map<number,{check:ReturnType<typeof checkProduct>;fresh:boolean}>();
  const details=await verifyCandidates(selected.map(p=>p.id),loader,p=>{
    const now=Date.now();observations.set(p.id,{check:checkProduct(p,options,now),fresh:fresh(p,now)});
  });
  ...
  let verified=confirmed.filter(p=>{
    const {check,fresh:wasFresh}=observations.get(p.id)!;
    ...
    if((sort==="price-asc"||sort==="price-desc")&&(!wasFresh||p.price===null||p.price<=0)){if(!options.price)unknownPrice++;return false;}
    if(sort==="stock"&&(!wasFresh||cityQuantity(p,options.city)===null)){if(!options.inStock)unknownStock++;return false;}
  ...
  const items=strict?verified.slice((page-1)*limit,page*limit).map(p=>({...p,cityQuantity:cityQuantity(p,options.city),stockCity:options.city||null,priceConfirmed:observations.get(p.id)!.fresh&&p.price!==null&&p.price>0})):
  ...
  return {query:options.query||"",page,limit,total,totalPages:Math.max(1,Math.ceil(total/limit)),items,
    technicalAvailable,coverage:{candidates:candidates.length,checked:selected.length,failed:details.filter(p=>!p).length,partial:(strict&&selected.length<candidates.length)||details.some(p=>!p),scope:strict?"verified-candidates":"catalog-page",unknownPrice,unknownStock,unknownTechnical},
