...
export type ChatProduct=Product & {verifiedAt?:string;facts?:Record<string,{conflict?:boolean;propertyValue?:string|number;textValue?:string|number}>;evaluation:{level:string;checks:Check[]}};
...
export async function sendChat(text:string){
  ...
  try{
    const data=await api<{message:string;conversationId:string;products:ChatProduct[];aiStatus:string}>("/api/assistant/chat",{
      method:"POST",body:JSON.stringify({message,conversationId:state.conversationId}),signal:controller.signal});
