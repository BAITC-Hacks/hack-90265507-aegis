...
export default function AssistantPanel(){
  ...
  return <section className="assistant-panel">
    ...
    {chat.aiStatus&&chat.aiStatus!=="ready"&&<div className="assistant-mode" role="status">
      {chat.aiStatus==="not_configured"?"AI не подключён: в backend/.env нужны OPENAI_API_KEY и OPENAI_MODEL. Сейчас работают локальный разбор условий, цены и наличие.":"AI временно недоступен. Ответ подготовлен по данным каталога."}
    </div>}
        ...
        <div className="assistant-message-content">
          ...
          {!!m.products.length&&<div className="assistant-products">{m.products.map((p,i)=><article key={p.id} className="assistant-product-card">
            ...
            <div className="assistant-product-footer"><div><strong>{p.price>0?p.price.toLocaleString("ru-RU")+" ₸":"Цена по запросу"}</strong>
              <span>{p.stockCity ? p.stockCity+": "+(typeof p.cityQuantity==="number"?p.cityQuantity+" шт.":"остаток не подтверждён") : typeof p.quantity==="number"?p.quantity>0?"В наличии: "+p.quantity:"Нет в наличии":"Остаток не указан"}</span>
    ...
    <div className="chat-shortcuts">
      <button disabled={chat.loading} onClick={()=>void sendChat("Сравни первые два",selectedCity)}>Сравнить варианты</button>
